...
// Generic compact-value transport codec (`compact-value-v1.base64url`, `MCV1`
// framing).
//
// Inlined verbatim from the monorepo's openid package (design D2): it is 134
// lines of protocol-neutral, stateless logic that depends only on `node:buffer`
// and `@midnight-ntwrk/compact-runtime`. The digital-passport family must not
// depend on a protocol transport package just for this codec.
//
// BIT-COMPATIBILITY OBLIGATION (permanent): wallets and verifiers encode on
// opposite sides of the wire, so this codec must remain byte-for-byte
// compatible with the core implementation's wire format. The ported conformance
// test (`src/test/compact-value-codec.test.ts`) is the guard; it is unchanged
// from the monorepo. If the codec ever diverges from core's wire format,
