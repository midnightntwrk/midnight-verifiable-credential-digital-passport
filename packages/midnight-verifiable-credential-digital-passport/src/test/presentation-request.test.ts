...
const modelSource = readFileSync(
  path.resolve(import.meta.dirname, '..', 'digital-passport-credential', 'model.compact'),
  'utf8',
);
...
describe('digital-passport presentation request', () => {
  ...
  it('rejects a presentation request with an empty verifier challenge', () => {
    ...
    expect(() =>
      pureCircuits.assertValidDigitalPassportPresentationRequest(emptyChallengeRequest),
  ...
  it('accepts a valid presentation request', () => {
    ...
    expect(() =>
      pureCircuits.assertValidDigitalPassportPresentationRequest(fixture.presentationRequest),
    ).not.toThrow();
  });
  ...
  it('rejects document-number reveal when the credential has a null document-number commitment', () => {
    ...
    const newClaimRoot = pureCircuits.digitalPassportClaimRoot(newClaimCommitments);
    ...
    const credentialProof = signProof({
      bodyRoot: pureCircuits.digitalPassportCredentialBodyRoot(credentialWithNullDoc),
      context: 'issuance',
    ...
    const presentationProof = signProof({
      bodyRoot: pureCircuits.digitalPassportPresentationBodyRoot(presentationRevealingDoc),
    ...
    ).toThrow(/Cannot reveal document-number when the credential has no document number/);
  });
  ...
  it('rejects when the verifier challenge does not match the presentation proof', () => {
    ...
    ).toThrow(/Presentation proof challenge does not match the request challenge/);
  });
  ...
  it('rejects a presentation with a disclosure opening that does not match the credential commitment', () => {
    ...
    const mismatchedPresentationProof = signProof({
      bodyRoot: pureCircuits.digitalPassportPresentationBodyRoot(mismatchedPresentation),
      context: 'presentation',
      signer: fixture.holder,
      createdAt: fixture.presentationProof.createdAt + 1n,
      challengeHash: fixture.presentationProof.challengeHash,
    ...
    ).toThrow(/Presentation last-name disclosure does not match the credential/);
  });
  ...
  it('rejects a presentation where the disclosed age threshold does not match the requested threshold', () => {
    ...
    const mismatchedThresholdProof = signProof({
      bodyRoot: pureCircuits.digitalPassportPresentationBodyRoot(mismatchedThresholdPresentation),
      context: 'presentation',
      signer: fixture.holder,
      createdAt: fixture.presentationProof.createdAt + 1n,
      challengeHash: fixture.presentationProof.challengeHash,
