...
function buildTestCredential(params: {
  ...
  const claimCommitments = {
    ...
    lastNameCommitment: pureCircuits.lastNameCommitment(pad64(params.lastName), lastNameOpening),
  ...
  const nonceScalar = mod(BigInt('0x' + crypto.randomBytes(32).toString('hex')));
...
describe('digitalPassportCredentialDescriptor', () => {
  it('round-trips through encode/decode with fresh credential', () => {
    ...
    expect(new Uint8Array(decoded.claimCommitments.firstNameCommitment)).toEqual(
      new Uint8Array(credential.claimCommitments.firstNameCommitment),
    );
    ...
    expect(new Uint8Array(decoded.claimCommitments.dateOfBirthCommitment)).toEqual(
      new Uint8Array(credential.claimCommitments.dateOfBirthCommitment),
    );
    expect(new Uint8Array(decoded.claimCommitments.documentNumberCommitment)).toEqual(
      new Uint8Array(credential.claimCommitments.documentNumberCommitment),
    );
    expect(new Uint8Array(decoded.claimCommitments.issuingStateCommitment)).toEqual(
      new Uint8Array(credential.claimCommitments.issuingStateCommitment),
    );
    expect(new Uint8Array(decoded.claimRoot)).toEqual(new Uint8Array(credential.claimRoot));
  });
  ...
  it('encoding preserves bodyRoot hash', () => {
    ...
    const originalBodyRoot = pureCircuits.digitalPassportCredentialBodyRoot(credential);
    const encoded = encodeDigitalPassportCredential(credential);
    const decoded = decodeDigitalPassportCredential(encoded);
    const decodedBodyRoot = pureCircuits.digitalPassportCredentialBodyRoot(decoded);
...
describe('digitalPassportProofDescriptor', () => {
  ...
  it('encoding preserves proof validity', () => {
    ...
    expect(() => pureCircuits.assertValidIssuanceContextProof(bodyRoot, decoded)).not.toThrow();
  });
});
...
describe('Joint credential + proof encoding', () => {
  it('both descriptors produce valid compact-value-v1.base64url payloads', () => {
    ...
    const decodedCredential = decodeDigitalPassportCredential(credentialEncoded);
    const decodedProof = decodeDigitalPassportProof(proofEncoded);
    const bodyRoot = pureCircuits.digitalPassportCredentialBodyRoot(decodedCredential);
