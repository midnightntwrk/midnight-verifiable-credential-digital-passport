 ...
 * `encodeCompactPayload` / `decodeCompactPayload`.
 *
 * The compact-value transport codec is inlined into this package
 * (`./internal/compact-value-codec.js`) rather than imported from the monorepo's
 * openid transport package — see design D2 and the bit-compatibility obligation
 * documented on the inlined module.
 *
...
const uint64 = new CompactTypeUnsignedInteger(BigInt('18446744073709551615'), 8);
...
const schemaRefDescriptor: CompactType<SchemaRef> = {
  alignment: () =>
    bytes32
      .alignment()
      .concat(bytes32.alignment().concat(uint16.alignment().concat(uint16.alignment()))),
    ...
    bytes32
      ...
      .concat(
        bytes32
          ...
          .concat(uint16.toValue(value.majorVersion).concat(uint16.toValue(value.minorVersion))),
      ),
};
...
const explicitHolderBindingDescriptor: CompactType<ExplicitHolderBinding> = {
  ...
  fromValue: (value: Value): ExplicitHolderBinding => ({
    holderVerificationMethodRef: verificationMethodRefDescriptor.fromValue(value),
  }),
...
const signatureDescriptor: CompactType<Signature> = {
  alignment: () => CompactTypeJubjubPoint.alignment().concat(CompactTypeField.alignment()),
  ...
  toValue: (value: Signature): Value =>
    CompactTypeJubjubPoint.toValue(value.r).concat(CompactTypeField.toValue(value.s)),
};
...
export const digitalPassportCredentialDescriptor: CompactType<DigitalPassportCredential> = {
  ...
  fromValue: (value: Value): DigitalPassportCredential => ({
    ...
    issuerVerificationMethodRef: verificationMethodRefDescriptor.fromValue(value),
                    ...
                    noStatusBindingDescriptor
                      ...
                      .concat(
                        uint64
                          ...
                          .concat(
                            CompactTypeBoolean.toValue(value.hasExpiration).concat(
                              uint64
      ...
      .concat(
        uint64
          ...
          .concat(
            bytes32
              .alignment()
              .concat(CompactTypeJubjubPoint.alignment().concat(signatureDescriptor.alignment())),
          ),
      ),
  fromValue: (value: Value): Proof => ({
    signerVerificationMethodRef: verificationMethodRefDescriptor.fromValue(value),
 ...
 */
export function decodeDigitalPassportProof(encoded: EncodedCompactValue): Proof {
