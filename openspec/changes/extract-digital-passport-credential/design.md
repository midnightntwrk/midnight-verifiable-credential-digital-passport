...
- Exact compact `include` specifier syntax accepted by 0.30.0 for package-installed sources (spike resolves it; fallback staged in D7/D8).
- Which RC version number core chooses for `midnight-did-credentials` (this repo just pins whatever ships).
- Whether the smoke lane lives in `ci.yml` or a dedicated workflow file — cosmetic; decide during implementation.
- DevDependency style at the workspace root (direct pins vs pnpm catalog) — follow midnight-did's style during scaffolding.
