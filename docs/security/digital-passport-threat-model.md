...
- **Issuer** — mints the credential; proves the issuance context with its
  Jubjub signing key, which the credential identifies only by DID
  verification-method reference (`issuerVerificationMethodRef`) — a
  reference the circuits compare but never resolve to the key itself (§6).
...
32-byte opening generated by the issuer at issuance and delivered to the
holder — alongside the claim values — in the issuance result's
`privateParts` (`protocol-model.compact`); from issuance on, values and
openings are holder-private. The commitments are hashed into a single
